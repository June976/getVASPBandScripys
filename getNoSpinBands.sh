...
grep "k-point     1" $OUTCAR_FILE_NAME -A $line_count > bds.txt
...
echo "kps energy(eV) energy_set_zero_fermi" > bds_energy.txt
...
do
	...
	do
		...
		do
			...
			#echo $nline
			energy=$(sed -n "$nline p" bds.txt | awk '{print $2}')
			posx=$(echo $posx_st $posx_ed $k $NBRCHKPTS | awk '{print $1+($2-$1)*($3-1)/$4}')
			str_one_line=$(echo $posx $energy $FERMI_ENERGY | awk '{print $1,$2,$2-$3}')
